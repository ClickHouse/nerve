...
| `agent.claude_credential_store` | string | `"auto"` | Where the Claude CLI subprocess keeps OAuth credentials (its own login and every MCP plugin's tokens). `auto` = the CLI's own choice: the macOS Keychain when the process can read the login keychain, otherwise `~/.claude/.credentials.json`. That choice is inherited from whoever launched Nerve — a gateway started from Terminal reads the Keychain, one started from an SSH/agent/background context reads the file — and a refresh-token rotation in one store invalidates the other store's copy, so a gateway restarted from a different context finds stale tokens, the CLI deletes them (`invalid_grant`), and every session reports "needs authentication". `file` pins the CLI to the file store in every launch context by putting a `security` shim first on the subprocess PATH (`$NERVE_HOME/bin/security`; it answers the CLI's `Claude Code…` keychain probes like a locked keychain and passes everything else to `/usr/bin/security`). macOS only; a no-op elsewhere. Re-authorize plugins with the shim on PATH so the grant lands in the file store: `PATH="$NERVE_HOME/bin:$PATH" claude mcp login <server-name>` (add `--no-browser` from a headless shell). Never copy tokens between the two stores — that ties them to one rotating lineage, which is exactly what breaks |
