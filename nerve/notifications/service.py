        ...
        async def _deliver(channel_name: str) -> str | None:
            ...
            try:
                if channel_name == "web":
                    ...
                    # No message id means nothing was posted — an unresolved
                    # target or a refused API call. Reporting "slack" anyway
                    # records the row as delivered for good, so the operator
                    # sees an answered-looking notification nobody ever saw.
                    if not msg_id:
                        return None
                    await self.db.update_notification(
                        notification_id,
